    ...
    >
      <nav className="p-4 text-[#3b3b39]">
        <div className="container mx-auto flex justify-between items-center flex-row-reverse">
          ...
          {/* Desktop: Language Switcher & Nav Items */}
          <div className="hidden md:flex items-center gap-5">
            {/* Navigation Items */}
            <ul className="flex space-x-3 items-center gap-5">
              <li>
              ...
              <li>
                <Link to="/products" className="hover:text-gray-400 font-ysab text-lg font-medium">
                  {t('products')}
                </Link>
              </li>
              <li className="relative">
                <Link to="/favorites" className="hover:text-gray-400 font-ysab text-xl font-bold">
                  {favorites.length > 0 && auth.isAuthenticated && (
                    <i
                      className="absolute -top-[1rem] right-0 text-xs font-medium font-sans text-white z-50 bg-red-500 w-[.9rem] h-[.9rem] rounded-full text-center"
                      style={isEnglish ? { left: '20px', lineHeight: '.9rem' } : { right: '20px', lineHeight: '.9rem' }}
                    >
                      {favorites.length}
                    </i>
                  )}
              ...
              <li>
                <p onClick={handleLogout} className="hover:text-gray-400 font-ysab text-lg font-normal cursor-pointer">
                  {t('logout')}
                </p>
              </li>
            </ul>
            ...
            >
              <div className="container mx-auto flex flex-col items-center p-4 relative">
                ...
                <ul className="flex flex-col space-y-6 text-xl text-black mt-16">
                  <li>
                    <Link to="/" className="hover:text-gray-400 font-ysab text-lg font-normal">
                      {t('home')}
                    </Link>
                  </li>
                  <li>
                    <Link to="/products" className="hover:text-gray-400 font-ysab text-lg font-normal">
                      {t('products')}
                    </Link>
                  </li>
                  <li className="relative">
                    <Link to="/favorites" className="hover:text-gray-400 font-ysab text-xl font-bold">
                      {favorites.length > 0 && auth.isAuthenticated && (
                        <i
                          className="absolute -top-[.9rem] right-[3.2rem] text-xs font-medium font-sans text-white z-50 bg-red-500 w-[.9rem] h-[.9rem] rounded-full text-center"
                          style={isEnglish ? { left: '20px', lineHeight: '.9rem' } : { right: '20px', lineHeight: '.9rem' }}
                        >
                          {favorites.length}
                        </i>
                  ...
                  <li onClick={handleLogout}>
                    <p className="hover:text-gray-400 font-ysab text-lg font-normal cursor-pointer">
                      {t('logout')}
                    </p>
                  </li>
                  <div className="mt-10">
